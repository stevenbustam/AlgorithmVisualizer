...
- **Real-time animation** of algorithm execution, with each operation (comparison, swap, visit, etc.) highlighted as it happens
- **Adjustable speed control** to slow down or speed up the visualization
- **Custom or randomized input generation** (array size, range, graph density, etc.)
- **Play / pause / step-through / reset controls** for frame-by-frame inspection
- **Side-by-side pseudocode panel** that highlights the current line being executed
- **Live stats** — comparisons, swaps, array accesses, time complexity display
- **Responsive canvas rendering** (bars, nodes, grids, or graphs depending on algorithm type)
...
- **Frontend:** React (or vanilla JS) + Canvas/SVG for rendering
- **Styling:** Tailwind CSS or CSS Modules
- **State/animation:** requestAnimationFrame or a small animation-step queue
- **Optional backend:** none required — fully client-side
...
algorithm-visualizer/
├── src/
│   ├── algorithms/       # sorting, searching, graph, pathfinding logic
│   ├── components/       # UI: controls, canvas, pseudocode panel, stats
│   ├── hooks/            # animation/state management
│   ├── utils/            # random data generators, timing helpers
│   └── App.jsx
├── public/
├── README.md
└── package.json
